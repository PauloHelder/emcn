        ...
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl border shadow-sm">
            ...
            <div className="flex bg-slate-100 p-1 rounded-xl">
              ...
              <button onClick={() => setView('PROGRAM')} className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${view === 'PROGRAM' ? 'bg-white shadow-sm text-emcn-blue' : 'text-slate-500 hover:text-slate-700'}`}>Cronograma</button>
              <button onClick={() => setView('EXAMS')} className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${view === 'EXAMS' ? 'bg-white shadow-sm text-emcn-blue' : 'text-slate-500 hover:text-slate-700'}`}>Avaliações</button>
            ...
            <div className="grid lg:grid-cols-4 gap-6">
              <div className="lg:col-span-1 bg-white rounded-2xl border shadow-sm p-4 h-fit">
                ...
                <div className="space-y-1">
                  {[...selectedClass.sessions].sort((a, b) => a.date.localeCompare(b.date)).map(s => (
                    <button
                      key={s.id}
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    <div className={`font-bold transition-colors ${isPresent ? 'text-emcn-blue' : 'text-slate-600'}`}>{student.name}</div>
            ...
            <div className="grid lg:grid-cols-4 gap-8">
              <div className="lg:col-span-1 space-y-4">
                <div className="bg-white p-6 rounded-3xl border shadow-sm">
                  <h4 className="font-bold text-slate-800 mb-4 px-1 flex items-center gap-2">
                    <FileText size={18} className="text-emcn-gold" /> Provas da Turma
                  </h4>
                  <div className="space-y-2">
                    {exams.map(exam => (
                      <button
                        ...
                        className={`w-full text-left p-4 rounded-2xl transition-all border-2 ${selectedExam?.id === exam.id ? 'border-emcn-gold bg-emcn-gold/5 shadow-md' : 'border-slate-50 hover:border-slate-100 text-slate-600'}`}
                      >
                        <div className="font-bold text-sm line-clamp-1">{exam.title}</div>
                        <div className="text-[10px] text-slate-400 font-medium mt-1">{new Date(exam.date).toLocaleDateString()}</div>
                      </button>
                    ))}
                    {exams.length === 0 && (
                      <div className="py-10 text-center text-slate-400 text-xs">
                        Nenhuma prova criada para esta turma ainda.
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="lg:col-span-3">
                {selectedExam ? (
                  <div className="bg-white rounded-[40px] border shadow-sm overflow-hidden animate-in fade-in slide-in-from-right-4 duration-500">
                    <div className="p-10 bg-slate-50 border-b flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                      <div>
                        <div className="text-[10px] font-black text-emcn-gold uppercase tracking-[0.2em] mb-2">Banco de Questões</div>
                        <h3 className="text-2xl font-bold text-emcn-blue">{selectedExam.title}</h3>
                        <p className="text-sm text-slate-500 mt-1 font-medium">{getDisciplineName(selectedExam.disciplineId)} • {selectedExam.subject}</p>
                      </div>
                      <button
                        onClick={() => {
                        ...
                        className="px-8 py-3 bg-emcn-blue text-white rounded-2xl font-bold text-sm hover:bg-slate-800 transition-all shadow-xl shadow-emcn-blue/20 flex items-center gap-2"
                      >
                      ...
                      {selectedExam.questions && selectedExam.questions.length > 0 ? (
                        <div className="space-y-6">
                          {selectedExam.questions.map((q: any, idx: number) => (
                            <div key={idx} className="bg-white p-8 rounded-[32px] border-2 border-slate-50 hover:border-slate-100 transition-all group">
                              <div className="flex justify-between items-start mb-6">
                                ...
                                <button
                                  onClick={async () => {
                                    ...
                                    const newQuestions = selectedExam.questions.filter((_: any, i: number) => i !== idx);
                                    const { error } = await supabase.from('exams').update({ questions: newQuestions }).eq('id', selectedExam.id);
                                    if (!error) {
                                      const updatedExam = { ...selectedExam, questions: newQuestions };
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pl-14">
                                {q.options.map((opt: string, optIdx: number) => (
                                  <div
                                    key={optIdx}
                                    className={`p-4 rounded-2xl border-2 text-sm font-medium flex items-center gap-3 ${optIdx === q.correctIndex ? 'border-green-100 bg-green-50 text-green-700' : 'border-slate-50 text-slate-500'}`}
                                  >
                                    <div className={`w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-black ${optIdx === q.correctIndex ? 'bg-green-600 text-white' : 'bg-slate-100 text-slate-400'}`}>
                                      {String.fromCharCode(65 + optIdx)}
                                    </div>
                                    {opt}
                                    {optIdx === q.correctIndex && <Check size={14} className="ml-auto" />}
                                  </div>
                                ))}
                              </div>
                            </div>
                        ...
                        <div className="py-20 text-center text-slate-300">
                          ...
                          <p className="text-lg font-medium">Nenhuma questão cadastrada para esta prova.</p>
                          <p className="text-sm mt-2">Clique no botão acima para começar a criar sua prova online.</p>
                        </div>
                      )}
                    </div>
                  </div>
                ) : (
                  <div className="h-full min-h-[400px] flex flex-col items-center justify-center bg-white rounded-[40px] border-2 border-dashed border-slate-200 text-slate-400 p-12">
                    <div className="w-24 h-24 bg-slate-50 rounded-full flex items-center justify-center mb-6">
                      <ListChecks size={48} className="opacity-10" />
                    </div>
                    <h4 className="text-2xl font-bold text-slate-800 mb-2">Selecione uma Prova</h4>
                    <p className="text-slate-400 font-medium text-center max-w-sm leading-relaxed">Escolha uma prova no menu lateral para visualizar e editar seu banco de questões.</p>
                  </div>
                )}
              </div>
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
                  <div className="bg-white rounded-[40px] w-full max-w-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
                    <div className="bg-emcn-blue p-8 text-white relative">
                      <h3 className="text-2xl font-serif">Nova Questão</h3>
                      <p className="text-white/60 text-sm mt-1">Defina o enunciado e as opções de resposta.</p>
                      <button onClick={() => setShowQuestionForm(false)} className="absolute top-8 right-8 text-white/50 hover:text-white transition-colors">
                        <X size={24} />
                      </button>
                    </div>
                    ...
                    <form onSubmit={async (e) => {
                      ...
                      setLoading(true);
                      try {
                        const newQuestions = [...(selectedExam.questions || []), { ...questionFormData }];
                        const { error } = await supabase.from('exams').update({ questions: newQuestions }).eq('id', selectedExam.id);
                        if (error) throw error;
                      ...
                      <div className="space-y-2">
                        ...
                        <textarea
                          ...
                          className="w-full px-5 py-4 bg-slate-50 border-2 border-transparent focus:border-emcn-gold focus:bg-white rounded-2xl outline-none transition-all font-medium min-h-[100px]"
                          placeholder="Digite aqui o enunciado da questão..."
                        />
                      </div>

                      <div className="space-y-4">
                        <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Opções e Resposta Correta</label>
                          ...
                          {questionFormData.options.map((opt, idx) => (
                            <div key={idx} className="flex gap-3">
                              <button
                                ...
                                className={`w-12 h-14 rounded-xl flex items-center justify-center font-bold transition-all border-2 ${questionFormData.correctIndex === idx ? 'bg-green-600 border-green-600 text-white shadow-lg shadow-green-200' : 'bg-slate-50 border-transparent text-slate-400 hover:border-slate-200'}`}
                              >
                              ...
                              <input
                                ...
                                className="flex-1 px-5 py-4 bg-slate-50 border-2 border-transparent focus:border-emcn-gold focus:bg-white rounded-2xl outline-none transition-all font-medium"
                      ...
                      <div className="pt-4 flex gap-4">
                        ...
                        <button
                          type="submit"
                          disabled={loading}
                          className="flex-1 px-8 py-4 bg-emcn-gold text-emcn-blue rounded-2xl font-bold hover:bg-[#b08e4d] transition-all shadow-lg shadow-emcn-gold/20 flex items-center justify-center gap-2"
                        >
                          {loading ? <Loader2 size={20} className="animate-spin" /> : 'Confirmar Questão'}
                        </button>
                      </div>
                    </form>
                  </div>
